...
### 결정
TripMate 계열 database는 `kor-travel-geo-postgres` 컨테이너의 `kor_travel_geo`, `tripmate`, `kor_travel_concierge`, `krtour_map` database로 통합하고, Docker 생명주기와 로컬 포트·credential·bucket 기본값은 `kor-travel-docker-manager`가 관리한다.
...
### 컨텍스트
TripMate 공용 인프라가 늘어나면서 Docker 컨테이너 상태뿐 아니라 시간에 따른 리소스 메트릭을 표준 관측 도구로 확인할 필요가 생겼다. 사용자는 Prometheus, Grafana, Exporter를 각각 별도 Docker 컨테이너로 올리고, 이후 Grafana를 다른 앱과도 공통 연계할 수 있도록 단일 관측 target이 아니라 개별 target으로 분리하기를 요청했다.

### 결정
단일 관측 target은 사용하지 않는다. Grafana(`tripmate-grafana`)는 `gra`, cAdvisor Exporter(`tripmate-cadvisor`)는 `cadv`, Prometheus(`tripmate-prometheus`)는 `prom` target으로 분리한다. dependency 순서는 `db -> storage -> gra -> cadv -> prom -> geo -> map -> ai -> main`이며, host 포트는 Grafana `12205`, cAdvisor `12301`, Prometheus `12401`을 사용한다. Compose service는 독립 실행 가능해야 하므로 Grafana와 Prometheus service 사이의 compose `depends_on`은 두지 않는다.
...
- Prometheus 관측 스택이 `kor-travel-geo` API 자체 성능 메트릭을 수집할 수 있다.

...
- Manager API가 root로 실행될 때 `Path.home()`이 `/root`가 되는 경로 drift를 막기 위해
  ...
  섞지 않고 `~/backups/legacy/<role>/`로 보존 이동했으며, active directory와 산출물은
...
- n150 host cron에 다음 UTC 시각의 wrapper를 설치했다: `geo_dagster` 03:15(keep 4),
...
- public Manager UI에서 Playwright live E2E를 실행했다. 로그인 200과 session 보호,
  백업 이력 200 및 세 role 표시, logout 200, logout 뒤 보호 API 401을 확인했다.
  backend root와 frontend static asset을 함께 배포해야 이 결과가 재현된다는 점도 확인했다.

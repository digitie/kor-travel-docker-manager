...
`tripmate-manager`는 불특정 다수에게 노출되는 범용 Docker 콘솔이 아니다. 목적은 `tripmate`, `tripmate-agent`, `python-kraddr-geo`, `python-krtour-map`이 의존하는 공용 Docker 인프라를 한 곳에서 확인하고 실행하는 것이다.

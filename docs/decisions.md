...
- `ktdctl pinvi-pair capture --verified-compatible`는 manifest가 없거나 legacy v1인 빈 환경에서도
  base dependency → Map API → Map dependents → PinVi API → PinVi dependents 순서로 전체 토폴로지를
  단계 기동하고 최초 v2를 원자 기록한다. 실패하면 두 API를 중지하고 이 transaction이 새로 만든
...
- Map과 PinVi image가 모두 새 계약을 이해해야 하므로 단일 서비스만 임의 rollback할 수 없다.
  기존 v2나 알 수 없는 manifest는 bootstrap capture로 덮어쓰지 않는다. legacy v1은 전체 candidate
  검증 성공 뒤에만 v2로 교체한다.

...
M05 one-shot launcher는 leaf와 one-shot ledger를 만들기 전에 M05 integration adapter의 read-only
source-pair preflight를 반드시 통과한다. 이 검사는 generic registry가 소유하지 않는 PinVi provenance의

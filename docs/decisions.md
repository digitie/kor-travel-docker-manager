...
manifest active/rollback 양쪽에만 있을 때 현재 UI admin proxy를 `없음 또는 frozen exact`로 허용한다.
한 번이라도 cursor hard-require가 있는 source v4 pair가 기록되면 active를 v3로 rollback한 뒤에도
현재 UI admin proxy exact를 요구한다. 알려진 v3/v4 shape 밖의 source와 부분 cursor 결선은 거부한다.
candidate raw/resolved와 activation 후 runtime 검사는 source v3에서도 새 결선을 항상 필수 exact로
유지한다. 이 source env 세대는 compatible-pair manifest 자체의 v4와 다른 개념이다.
...
- base runtime에서 새 manager 결선으로 가는 최초 managed cutover/rollback은 허용하면서, source
  env v4 runtime의 admin proxy 누락은 mutation 전에 차단한다.

...
**문제**: DeploymentContractError raise가 세 서비스 파일에만 508곳인데 계층이 평평해 분류가 메시지 문자열 매칭으로 이뤄진다: compose_service.py:5285는 `str(exc) == "PinVi sealed role topology is noncanonical"`으로 terminal(재시도 금지) 여부를 정하고, cli.py:1006은 `"no backup" in str(exc)`, cli.py:1242는 `"pair rotation" in str(exc)`로 exit code를 정한다. 문구 하나를 다듬으면 terminal이 재시도 가능으로 조용히 오분류된다. `ktdctl pinvi-pair --json`은 실패 시 {"status":"failed","classification":"unclassified"}만 내고 메시지를 버리며(cli.py:210-218) **[해소: work/post-e2e17 — 분류가 셋(prejournal_failure/postjournal_failure/unclassified)으로 갈라지고, 봉인 stage와 실패 서비스가 payload에 남는다. 인용된 줄번호는 이동했다.]**, cli.py:996-997이 문서화한 '--json은 어떤 경로에서도 stdout에 JSON만' 계약을 `pin rotate`(774-776)·`rotate-pair`(830-832)·block·rollback·init·verify는 어겨 stderr 텍스트만 내고 stdout이 비어 `| jq` 스크립트가 죽는다.
...
**구현**: 검증 노트가 확정한 축소 범위만 구현했다. (1) **pinned pair role**: 새 `PINNED_ROLES` tuple을 만들지 않고, 이미 있는 정본 `pinned_runtime_release.RUNTIME_SOURCE_ROLES`/`RuntimeSourceRole`에 두 곳만 배선했다 — `cli.py`의 `pin rotate --role` `choices`를 `list(RUNTIME_SOURCE_ROLES)`로, `routes.py`의 `RuntimePinRotationRequestBody.role` 타입을 `RuntimeSourceRole`로. "pinned 대상 추가가 config 등록만으로 끝난다"는 원안 benefit은 ADR-40(pair 구조 동결)에 따라 철회했고, frontend(api.ts/RuntimePinPanel.tsx)의 리터럴 복제는 검증 노트가 실제 배선 범위에서 제외했으므로 손대지 않았다. (2) **백업 role**: 백엔드는 이미 단일 정본(`standalone_backup.BACKUP_ROLES`)이므로 스키마 변경 없이, `GET /api/v1/backups` 응답에 `"roles": list(BACKUP_ROLES)` 필드만 추가했다(role 필터가 걸려도 항상 전체 canonical 목록 — "지금 보는 필터"와 "고를 수 있는 목록"은 다른 질문이다). 프론트 `BackupHistoryPanel.tsx`는 하드코딩된 `ROLE_OPTIONS` 배열을 제거하고 이 `roles` 필드에서 select/생성 버튼·신선도 표를 파생하도록 바꿨고, `api.ts`의 `StandaloneBackupManifest.role` 타입은 하드코딩 union에서 `string`으로 완화했다(백엔드가 유일한 정본이므로 새 role 추가 시 이 파일을 고칠 필요가 없어진다). `EXPECTED_INTERVAL_HOURS`(cron 신선도 정책)는 검증 노트 (4)가 지적한 대로 config에서 파생 불가능한 별도 정책이라 그대로 남겼다. docker-targets.yml connection 문자열의 "(DBs: ...)" cosmetic 중복도 검증 노트 (3)에 따라 손대지 않았다. 회귀 테스트: CLI 레벨(`test_pin_rotate_role_choices_come_from_the_canonical_runtime_source_roles`)과 API 레벨(`test_post_runtime_pin_request_rejects_a_role_outside_the_canonical_set`) 2건을 신설했고, 기존 백업 목록 테스트 2건에 `roles` 필드 검증을 추가했다 — 4건 모두 원복 후 실패를 확인하는 mutation 검증을 거쳤다. 프론트는 `tsc --noEmit`/`next lint`/`next build` 전부 통과.

**구현 후 적대적 리뷰 반영** (2명, 독립): 두 리뷰어 모두 같은 두 가지를 지적했다. (1) **회귀 테스트의 실제 보호 범위가 과장돼 있었다**: CLI·API 두 신설 테스트는 "정본 값과 현재 일치하는가"만 확인해서, `RuntimeSourceRole`이 `Literal["map", "pinvi"]`인 지금은 실제 배선과 우연히 같은 값을 가진 독립 하드코딩을 구분하지 못한다 — 리뷰 A·B 둘 다 `cli.py`의 choices를 원래 `["map", "pinvi"]` 하드코딩으로 되돌려도 신설 CLI 테스트가 그대로 통과함을 실측으로 확인했다(재현·복구 완료). "4건 모두 mutation 검증을 거쳤다"는 커밋 메시지 문구는 이 두 건에는 과장이었다. CLI 쪽은 고칠 수 있어 고쳤다: `test_pin_rotate_role_choices_track_the_canonical_tuple_dynamically`를 추가해 `kor_travel_docker_manager.cli.RUNTIME_SOURCE_ROLES` 자체를 monkeypatch로 다른 값으로 바꿔치기하고 `build_parser()`가 그 값을 실제로 따라가는지 확인한다(하드코딩으로 되돌리면 이 테스트가 실패함을 실측 확인). API/Pydantic 쪽은 구조적으로 고칠 수 없다는 것을 확인했다 — `RuntimePinRotationRequestBody`의 클래스 본문이 import 시점에 한 번 평가되므로 monkeypatch가 이미 만들어진 필드 타입에 영향을 주지 못하고, 대안으로 검토한 `model_fields["role"].annotation is RuntimeSourceRole` identity 검사도 CPython의 `typing.Literal`가 `_tp_cache`로 동일 인자·동일 순서 호출을 같은 객체로 캐싱함을 실측으로 확인해(`Literal["map","pinvi"] is Literal["map","pinvi"]` → `True`) 무력하다 — 오늘 이 필드를 독립적으로 `Literal["map", "pinvi"]`라고 다시 적어도 캐싱 때문에 정본과 같은 객체가 되어 이 검사를 통과한다. 이 경우의 실질 보호는 코드 리뷰/grep으로만 가능한 구조적 한계이며, 새 테스트로 가릴 수 없어 여기 명시한다. (2) **프론트 role 파생 로직에 자동 테스트가 전혀 없었다**: 리뷰 A·B 둘 다 `BackupHistoryPanel.tsx`의 role 파생부를 옛 하드코딩 방식으로 되돌려도 `tsc`/`lint`/기존 vitest 스위트가 전부 통과함을 실측으로 확인했고, 이 회귀의 실사용자 영향은 조용하다(백엔드가 새 role을 추가해도 이 UI만 영구히 선택 불가·신선도 표 누락으로 남고 에러가 없다) — 이 태스크 자체가 없애려던 "조용한 층간 불일치" 버그류가 "하드코딩 배열"에서 "안전망 없이 재도입될 수 있는 하드코딩 배열"로 자리만 옮긴 셈이었다. `vitest.config.mts`가 "컴포넌트 렌더 테스트는 필요해지면 그때 jsdom 도입" 정책을 명시하므로 그 경계를 넘지 않고, role 파생을 순수 함수 `frontend/src/lib/backupRoles.ts`의 `buildBackupRoleOptions`로 추출해 기존 순수 로직 전용 vitest 설정 그대로 `backupRoles.test.ts` 3건을 추가했다(옛 하드코딩 목록에 없던 canary role 이름으로 회귀를 검증 — 하드코딩으로 되돌리면 실패함을 실측 확인). 컴포넌트는 이 함수를 호출하도록 배선만 바꿨다. 그 외 두 리뷰가 공통으로 확인한 것: `pinned_runtime_release.RUNTIME_SOURCE_ROLES`/`RuntimeSourceRole`은 이 커밋 이전(각각 `10662fb`/`645fd25d`)에 이미 존재했다는 전제가 `git blame`으로 정확했고, `routes.py`에서 제거한 `Literal` import는 실제로 무사용이었으며, import 순환 없음, 응답 envelope 변경(`roles` 필드 추가)이 다른 테스트의 exact-equality를 깨지 않음, `StandaloneBackupManifest.role` 완화가 다른 파일의 union exhaustiveness에 영향 없음을 모두 재확인했다. 리뷰 A가 별도로 짚은 세 번째 항목(아래 별도 기록)과, `roles` 로딩 전 짧은 빈 상태 UX(Low, 하드코딩 대체는 이 개선 자체의 목적에 반해 고치지 않음), `key='all'` 충돌 가능성(Nit, `BACKUP_ROLES`가 정적 tuple이라 현재 도달 불가)은 그대로 남긴다.

**리뷰가 찾은, 이 커밋 범위 밖의 잔여 항목**: 두 리뷰 모두 독립적으로 세 번째 role 리터럴 중복 사이트를 발견했다 — `backend/src/kor_travel_docker_manager/services/m05_isolated_harness.py:26`의 `M05IsolatedRuntimeRole = Literal["map", "pinvi"]`(30/33/126/141/145/225에서 실사용, `PinnedRuntimeRelease`는 import하지만 `RUNTIME_SOURCE_ROLES`/`RuntimeSourceRole`은 import하지 않음). 이전 세션 판단과 달리 harness/test 전용이 아니라 `scripts/m05_isolated_e2e.py`(root-only one-shot M05 격리 bridge E2E driver)를 통해 실제 production 경로에서 도달 가능함을 두 리뷰 모두 재확인했다. 다만 검증 노트가 명시한 이번 배선 범위(cli.py·routes.py 두 곳)에는 애초에 포함되지 않았고, ADR-40이 pair를 정확히 두 role로 동결하고 있어 실질 드리프트 위험은 낮다는 데에도 두 리뷰가 동의한다 — 이번 커밋의 결함은 아니다. 다만 이 파일이 M05 동결 계약 인접 코드라 이번 세션의 기존 관례(M05 관련 파일은 배선 전에 별도 확인)를 따라 이번에는 손대지 않고 `docs/tasks.md`에 후속 항목으로만 남긴다.
...
**구현**: 검증 노트의 두 정정을 그대로 따르되, 구현 중 노트 자체의 범위 오류를 하나 더 발견해 바로잡았다. (1) **c6c_deployment.py**: 검증 노트가 지목한 5개 함수(validate_resolved_compose_secret_isolation·validate_compose_env_file_isolation·run_map_ops_smoke·run_map_ui_auth_preflight+run_ui_auth_smoke·require_local_c6c_image, ~639줄)를 삭제했다. map-dataset 검증 서브트리는 노트가 말한 2단(`_validate_map_dataset_row`→`_validate_map_dataset_identity`)보다 한 단 더 깊었다 — `ruff`의 F821(정의되지 않은 이름)이 삭제 직후 `_validate_map_datasets_envelope`가 이미 죽은 `_validate_map_dataset_row`를 호출하고 있음을 잡아냈고, 재확인 결과 `_validate_map_datasets_envelope` 자신도 호출자가 0건이었다. 즉 서브트리는 `_validate_map_datasets_envelope`→`_validate_map_dataset_row`→`_validate_map_dataset_identity`→`_map_dataset_detail_url` 4단이었다(마지막 두 개만 노트가 알고 있었다) — 4개 전부 삭제했고, `_validate_map_dataset_identity`를 직접 단위 테스트하던 `test_c6c_cancel_probe_resume.py`의 `test_map_dataset_identity_uses_exact_membership_triple`도 함께 제거했다. (2) **compose_service.py**: 노트가 한정한 3개 함수(`_clean_repository_revision`·`_c6c_source_snapshot_environment`·`_export_git_tree`, tarfile.extractall 포함)와 `get_c6c_deployment_lock_path`를 삭제하고 `_resolve_repository_path`는 그대로 남겼다. 기계적 line-range 삭제 과정에서 두 차례 데코레이터 경계 실수가 있었다 — `get_c6c_deployment_lock_path` 삭제 범위가 바로 뒤 `ComposeEnvFileIdentity`의 `@dataclass(frozen=True)`까지 삼켜 그 dataclass가 필드 없는 빈 클래스가 됐고, `_c6c_source_snapshot_environment`의 `@contextmanager`가 삭제 범위 밖에 남아 바로 다음 함수 `_run_git_read`에 잘못 얹혔다. 둘 다 전체 테스트 스위트 1회 실행에서 32건 실패로 즉시 드러났고(`ComposeEnvFileIdentity() takes no arguments` 등), `git diff`로 원본과 대조해 정확한 위치에 복구한 뒤 재실행해 0 실패를 확인했다 — 대규모 기계적 삭제 뒤에는 반드시 전체 스위트로 검증해야 한다는 근거가 하나 더 늘었다. (3) **프론트 더미 의존성**: `DashboardClient.tsx`의 `useForm`/`z` import와 두 dummy 참조 줄을 제거하고 `package.json`의 `react-hook-form`/`zod`를 삭제, `npm install`로 `package-lock.json`을 갱신했다(다른 파일에서 두 라이브러리를 참조하지 않음을 grep으로 재확인). (4) **port_policy**: 개선안의 두 옵션 중 삭제를 선택했다(승격은 새 검증 기능 추가라 죽은 코드 제거 태스크 범위를 벗어난다고 판단) — `docker-targets.yml`의 `port_policy` 블록을 지우고 `docs/ports.md`가 이미 완결된 유일 정본임을 확인했다(registry.py 스키마 검증은 이 블록을 요구하지 않음, 다른 문서도 참조하지 않음). (5) **require_public_api_key**: 개선안의 두 옵션 중 "/metrics에 선택 적용"을 선택했다(관리 UI 키 흐름을 flag 뒤로 숨기는 대신, 기존 게이트에 실제 소비처를 만드는 쪽이 순삭제 없이 가짜 보안 어포던스를 해소한다) — `main.py`에 `KTDM_METRICS_REQUIRE_KEY` env를 매 요청 시점에 읽는 `_metrics_auth_gate` dependency를 추가했다(import 시점 고정이면 테스트가 모듈을 재로드해야 해서 요청 시점 평가로 설계). 기본값(미설정)은 기존 무인증 동작을 그대로 유지해 Prometheus scrape(127.0.0.1 전용)를 깨지 않는다. `decisions.md:625`의 open 항목을 종결로 갱신하고 `.env.example`에 새 변수를 문서화했다. 회귀 테스트 4건(기본 무인증 200, opt-in 시 무자격 401, 유효 키로 200, admin 세션으로 200) 신설, 배선 제거 mutation 검증 완료(401 테스트가 정확히 실패함을 확인 후 복구). 백엔드 전체 스위트 1393 passed(회귀 0), ruff 통과. 프론트 `tsc --noEmit`/`next lint`/`vitest`/`next build` 전부 통과.

**구현 후 적대적 리뷰 반영** (2명, 독립): Critical/High/Medium 발견 0건 — 두 리뷰 모두 14개 삭제 심볼 전체를 저장소 전체 재검색으로 독립 재확인했고(문서 자기 언급 제외 잔존 참조 0건), c6c_deployment.py의 9개 삭제 범위 전체를 데코레이터 경계 관점에서 직접 diff-audit해 compose_service.py에서 실제로 발생했던 것과 같은 종류의 사고가 여기서는 없었음을 확인했다(리뷰 A는 parent/current 버전의 모든 top-level `@decorator`→정의 쌍 11개를 스크립트로 대조해 추가·누락·불일치 0건으로 결론). `_resolve_repository_path`는 두 리뷰 모두 원본과 바이트 단위로 동일함을 diff로 확인했다. `/metrics` 게이트는 리뷰 A가 배선을 직접 mutation-test(제거 후 401 테스트만 실패 확인, 복구)했고, 리뷰 B가 `KTDM_METRICS_REQUIRE_KEY` 값 변형(`"true"`/`"TRUE"`/`" 1"`/`"0"` 등)과 잘못된 `key` 파라미터가 500이 아니라 항상 깨끗한 401로 처리됨을 확인했다. Low 2건을 반영했다: (1) 리뷰 B — `_metrics_auth_gate`가 `secure_state_file.insecure_mode_allowed`와 동일한 `.strip() == "1"` 파싱 규칙을 인라인으로 재작성하고 있었다. `insecure_mode_allowed`라는 이름을 강화(hardening) 토글에 그대로 재사용하면 호출부가 의미상 거꾸로 읽히므로(요청: "insecure_mode_allowed"가 참일 때 보안을 켠다는 모순), 중립적인 이름의 `env_flag(env_name)`를 `secure_state_file.py`에 신설해 파싱 규칙을 그 안에 두고 `insecure_mode_allowed`는 `env_flag`에 위임하는 얇은 wrapper로 바꿨다(기존 7개 회귀 테스트가 wrapper를 그대로 거쳐 가므로 변경 없이 계속 통과). `main.py`는 `env_flag`를 직접 가져와 쓴다. (2) 리뷰 A — GM-19 검증 노트가 `_resolve_repository_path`의 살아있는 근거로 인용한 `test_f1d_compose_contract.py:2139`(`test_candidate_preflight_rejects_a_build_context_outside_staged_source`)는 실제로는 `_map_source_environment_contract_version` 자체를 `monkeypatch.setattr`로 치환해 그 mock 호출 인자만 검증하므로, `_resolve_repository_path`를 직접 실행하지 않는다 — 이 인용은 노트 작성 시점부터 부정확했던 것으로, 이번 커밋이 만들거나 악화시킨 문제는 아니고(함수를 지우지 않은 결론 자체는 두 리뷰 모두 다른 근거로 옳다고 확인) `_resolve_repository_path`에 실제 커버리지가 있는지는 별도로 재확인이 필요하다는 점만 여기 남긴다. 회귀 테스트 재실행(11개 관련 테스트 + 전체 스위트) 통과, ruff 통과.
...
**구현**: 검증 노트가 확정한 "1단계" 네 가지를 그대로 구현했다(후속 c6c_deployment 4분할·compose_service 3분할·rebuild_pinned_runtime 테이블화는 명시적으로 범위 밖). (1) **services/errors.py**: `DeploymentContractError`·`ComposeCandidateContractError`·`ComposePostMutationContractError`(GM-06의 `code` 필드 포함)를 그대로 옮겼다. `c6c_deployment.py`는 하위 호환을 위해 이 세 클래스를 재수출하는데, `ComposePostMutationContractError`는 그 파일 내부에서 쓰이지 않아 `ruff`가 "미사용 import"로 잡았다 — 재수출 5개(에러 3종+capability 센티널 2종)만 나열하는 `__all__`을 파일 끝에 추가해 해결했다(`metrics_collector.py`의 기존 3-이름 `__all__`과 같은 관례 — 이 거대 모듈의 다른 공개 이름 전부를 나열할 필요는 없다; 최초 구현은 `import X as X` 관용구를 썼으나 리뷰 반영으로 이 관례에 맞춰 바꿨다). `compose_service.py`·`main.py`도 이제 `errors.py`에서 직접 가져온다. (2) **services/capabilities.py**: `_MANAGED_COMPOSE_MUTATION_CAPABILITY`·`_PINNED_RUNTIME_REBUILD_MUTATION_CAPABILITY` 센티널(과 그 클래스)을 옮기고 `c6c_deployment.py`가 재수출한다. `is` identity 비교로 게이트되므로 재수출이 진짜 같은 객체를 가리키는지가 핵심이라, 이를 직접 확인하는 회귀 테스트를 신설했다(아래). `compose_service.py`·`docker_service.py`는 이제 `capabilities.py`에서 직접 가져온다 — compose_service.py의 밑줄 10개 크로스 import 중 이 2개만 해소했고 나머지 8개(서비스 이름 상수 6개, 헬퍼 함수 2개)는 검증 노트가 지목한 범위 밖이라 그대로 남겼다. (3) **transaction 캡처 공개 승격**: `_capture_transaction_unlocked`→`capture_transaction_unlocked`, `_capture_candidate_transaction_unlocked`→`capture_candidate_transaction_unlocked`로 밑줄만 뗐다(`compose_service.py` 내부 호출 8곳, `docker_service.py` 외부 호출 2곳, 테스트 3개 파일의 monkeypatch/patch 참조 전부 함께 변경). 검증 노트가 제시한 두 옵션(lock snapshot을 인자로 요구 / 시그니처·문서로 선행조건 명시) 중 후자를 골랐다 — GM-19에서 sed 기반 line-range 삭제가 데코레이터 경계를 두 번 잘못 잘랐던 사고 직후라, 여러 호출 체인에 새 필수 인자를 꿰어 넣는 쪽(전자)은 이번 "1단계"치고 위험 대비 이득이 낮다고 판단했다. 대신 각 메서드에 "호출자가 이미 lock을 들고 있어야 한다"는 선행조건을 명시하는 docstring을 추가했다 — 인자로 강제하는 안은 후속 태스크로 남을 수 있다. (4) **metrics_collector↔docker_service 순환 해소**: `metrics_collector.py`는 `MANAGED_CONTAINERS`를 `docker_service`의 재수출이 아니라 정본인 `registry.py`에서 직접 가져오고, `docker_service`를 아예 import하지 않는다. Docker client 접근은 `MetricsCollector.set_docker_client_provider(provider)`로 주입받는 콜백 방식으로 바꿨다 — `docker_service.py`가 자기 싱글턴 생성 직후(`docker_service = DockerService()` 바로 뒤) `metrics_collector.set_docker_client_provider(docker_service._get_client)`로 배선한다. `metrics_collector`가 더 이상 `docker_service`를 참조하지 않으므로 이 방향의 import는 순환이 아니고, `docker_service.py`의 기존 지연 import 2곳(545/830)을 평범한 모듈 레벨 import로 되돌릴 수 있었다. provider가 아직 배선되지 않은 상태(테스트에서 `MetricsCollector()`를 직접 생성한 경우 등)와 "Docker daemon이 실제로 죽은" 상태를 구분해야 인프라 장애처럼 보이는 배선 버그를 조용히 감추지 않는다 — 전자는 `RuntimeError`로 즉시 드러나게 했다. `tests/test_prometheus_metrics.py`가 `docker_service._get_client`를 monkeypatch하던 것을 `collector.set_docker_client_provider(...)` 직접 주입으로 갱신했다. 회귀 테스트: `tests/test_service_layer_boundaries.py` 신설(7건) — 에러/capability 재수출이 진짜 같은 객체인지(값 동등이 아니라 `is`), metrics_collector.py 소스에 `docker_service` import가 재도입되면 실패하는 AST 기반 검사, `MANAGED_CONTAINERS`가 registry.py 직접 참조인지, docker_service.py 로드가 실제로 provider를 배선하는지, 미배선 시 `RuntimeError`가 나는지, transaction 캡처 메서드가 공개인지를 각각 확인한다. 전부 원복 후 실패를 확인하는 mutation 검증을 거쳤다(특히 순환 재도입 mutation은 테스트 실패가 아니라 실제 `ImportError`로 앱 자체가 죽는 것까지 확인했다). 백엔드 전체 스위트 1464 passed(회귀 0, 신규 7건 포함), ruff 통과. 이 태스크는 백엔드 전용이라 프론트엔드 변경 없음.

**구현 후 적대적 리뷰 반영** (2명, 독립): Critical/High 발견 0건. (1) 리뷰 B가 실제 결함(Medium)을 찾았다 — 두 공개 메서드의 docstring이 선행조건을 "`c6c_deployment_lock_from_environment()` 컨텍스트 안"으로 못박았는데, `rebuild_pinned_runtime`(compose_service.py) 경로는 그 함수를 전혀 거치지 않고 `_pinned_runtime_rebuild_environment_lock()` 안에서 `c6c_deployment_lock(lock_snapshot.lock_path)`를 직접 잡아 같은 flock에 도달한다 — 실제 불변식(host lock 보유)은 이 경로도 만족하지만, docstring이 틀린 함수 이름을 지목해 문서 자체를 신뢰하는 미래 독자가 "이 경로는 조건을 안 지킨다"고 오판할 수 있었다. 두 경로 모두를 설명하도록 docstring을 고쳤다(호출자가 실제로 그 lock을 쥐고 있는지 코드로 직접 추적해 리뷰의 지적이 정확함을 재확인한 뒤 수정). (2) 리뷰 A가 커밋 메시지의 과장을 잡았다 — "순환 재도입 mutation이 테스트 실패가 아니라 실제 ImportError로 앱이 죽는 것까지 확인했다"는 문구는, `metrics_collector.py`가 프로세스에서 **가장 먼저** import되는 경우에만 해당하고, 실제 `main.py`의 import 순서(`docker_service.py`가 `metrics_collector.py`보다 먼저 완전히 로드됨)에서는 순환이 조용히 풀려 앱이 정상 기동한다는 것을 리뷰가 직접 재현해 반증했다. 다만 리뷰 A도 확인했듯 실제 회귀 보호는 이 import 순서와 무관하다 — `test_metrics_collector_module_does_not_import_docker_service`는 소스를 `ast`로 파싱해 검사하므로 import 순서와 상관없이 재도입을 결정적으로 잡는다(리뷰가 직접 재현). 즉 테스트의 보호력 주장은 정확했고, 커밋 메시지의 "앱 크래시" 서술만 과장이었다 — 코드 수정 없이 이 문서에 정정만 남긴다. (3) 리뷰 A가 스타일 지적(Low)을 했다 — `c6c_deployment.py`의 `import X as X` 재수출 관용구가 같은 커밋이 손댄 `metrics_collector.py` 자신의 기존 3-이름(비전수) `__all__` 관례(그리고 `job_runner.py`·`admin_password_service.py` 등)와 어긋난다는 지적을 받아들여 `__all__`로 교체했다(위 **구현** 문단에 반영). (4) 두 리뷰 모두 "lock 선행조건을 docstring으로만 보존한 것"을 잔존 위험으로 짚었지만 둘 다 검증 노트가 이미 인정한 트레이드오프이고, 현재 9개 호출부 전부가 실제로 lock 안에 있음을 각자 독립적으로 코드 추적해 확인했다(worst case도 조용한 손상이 아니라 `_assert_transaction_matches_c6c_lock`이 잡는 명시적 거부) — 추가 조치 없음. (5) 두 리뷰 모두 별도로, 같은 물리 워크트리에서 리뷰 mutation test를 동시에 실행하면 서로의 파일 변경이 순간적으로 섞여 보일 수 있음을 실측으로 재확인했다(GM-18 리뷰에서 처음 관찰된 것과 동일한 현상, 이번이 세 번째) — 양쪽 다 최종적으로 자기 변경을 원복해 실제 결함으로 이어지지는 않았지만, 다음 라운드부터는 리뷰 에이전트에 "실행 파일을 직접 수정하는 mutation test 대신 별도 프로세스의 monkeypatch/임시 스크립트로 검증하라"고 명시하거나 리뷰마다 별도 워크트리를 주는 편이 안전하다. 리뷰 반영 후 재검증: 관련 docstring 수정, `__all__` 전환, 전체 스위트 1464 passed·ruff 통과 재확인.

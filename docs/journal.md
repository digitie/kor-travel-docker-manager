...
n150의 승인된 rebuild는 source provenance 및 PinVi role credential preflight를 통과한 뒤, 기본
Compose candidate가 네 Map fence/permit mount path를 빈 `.env` 값으로 해석하면서 DB reset 전에
fail-close했다. 이 path들은 현재 pinset state root에서 결정론적으로 도출되고, 이후 candidate
override에도 같은 값으로 전달된다. 따라서 운영자가 pinset별 artifact path를 설정할 문제가 아니라
Manager가 source materialize·image build·journal·DB mutation 전에 directory를 준비해 기본 candidate에

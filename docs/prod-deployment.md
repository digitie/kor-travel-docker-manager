...
service 환경에는 주입하지 않는다. username은 runtime 값과 smoke 로그인 값이 하나의 source를 공유한다.
...
environment snapshot에서만 해석하며, Map UI의 정확한 세 runtime Env 경로 밖 노출과 평문 비밀번호
주입을 거부한다.
...
Env path에만, Map UI username·hash·session secret이 Map UI의 정확한 Env path에만 존재하는지 검사한다.
평문 UI smoke 비밀번호가 어떤 container scalar에도 존재하면 실패한다.

...
4. **포트 충돌 유발 금지**: PostgreSQL(`12500`/`12600`/`12700`/`12800` — geo/concierge/map/pinvi 전용 instance, ADR-37; `11000` — 공용 제어 평면 instance, ADR-44/ADR-45. concierge는 ADR-44로 `11000`쪽 role/database로 **이전을 마쳤고**(2026-09-19/20) `12600` instance는 롤백 보관용으로 남는다. geo는 ADR-45로 이전 준비(role/database 생성 코드)까지 끝났고 실제 data cutover는 별도 배포 단계다 — 그때까지 `12500` instance가 계속 활성이다. map/pinvi는 ADR-37 그대로 자기 instance를 쓴다), RustFS(`12101`, `12105`), Grafana(`12205`), cAdvisor(`12301`), Prometheus(`12401`), `kor-travel-geo`(`12501`, `12505`), `kor-travel-concierge`(`12601`, `12602`, `12605`), `kor-travel-map`(`12701`, `12702`, `12705`), PinVi(`12801`, `12805`), Manager(`12901`, `12905`) 포트는 Kor Travel/PinVi 구성 프로그램이 공용으로 접근할 수 있어야 하므로 임의로 변경하지 않는다.
...
9. **포트 정책 우회 금지**: 새 로컬 서비스 포트는 `docs/ports.md`의 `12000 + dependency index * 100 + offset` 규칙을 따른다. **PostgreSQL은 프로젝트마다 전용 instance를 쓰고 포트는 각 대역의 `x00`이다(ADR-37)** — map/pinvi는 지금도 이 규칙 그대로다. **concierge는 예외로, `11000` 공용 제어 평면 instance로 이전을 마쳤다(ADR-44, 2026-09-19/20). geo도 예외 대상으로 확정됐고(ADR-45) role/database 생성까지 끝났으나, 실제 data cutover는 아직이다** — `11000`은 target별 100단위 대역 밖의 별도 포트이며, 새 프로젝트가 이 instance에 합류해도 이 포트 자체는 바뀌지 않는다. 표준 `5432`를 쓰던 통합 instance는 2026-08-17에 폐지했다.

...
### 컨텍스트
`tripmate`, `tripmate-agent`, `python-krtour-map`, `python-kraddr-geo`가 같은 PC에서 PostgreSQL/RustFS 포트를 함께 쓰면서, 각 저장소가 개별적으로 컨테이너를 정지/재시작하면 포트 점유와 credential 기준이 충돌할 수 있다.

### 결정
TripMate 계열 database는 `kraddr-geo-postgres` 컨테이너의 `kraddr_geo`, `tripmate`, `tripmate_agent`, `krtour_map` database로 통합하고, Docker 생명주기와 로컬 포트·credential·bucket 기본값은 `tripmate-manager`가 관리한다.
...
### 후속
- (open) `tripmate`, `tripmate-agent`, `python-kraddr-geo`, `python-krtour-map` 레포에서 정책 포트로 설정과 문서를 순차 반영한다.

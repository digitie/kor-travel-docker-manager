...
candidate build 전에는 manifest active/rollback 합집합에 manager 전용 content-addressed local retention
tag를 만들고 exact ID를 재검증한다. build된 candidate도 첫 container stop 전에 같은 방식으로 보존한다.
이 reference는 moving service tag 교체와 기존 container 제거 뒤에도 manifest image가 남게 하며, 부분 tag
실패는 container mutation 전에 중단한다. manifest commit 뒤에는 새 두 슬롯 밖의 manager 전용 tag를
정리하고, cleanup residue가 있으면 다음 pair mutation 전에 해소한다. Map

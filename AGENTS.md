...
## 과결박 금지 — 보안모델·이미지 정합성 (ADR-50)

위 "Simplicity First"가 코드 크기에 대한 것이라면, 이것은 **설계 결박의 개수**에 대한
상시 원칙이다. 정본은 `docs/decisions.md` ADR-50.

- 보안모델은 단순하게 간다 — role 분리·상호 배타 게이트·봉인 digest·attestation
  체인을 겹겹이 쌓지 않는다. 단일 scoped 주체(geo 패턴)가 기본형이다.
- Manager가 이미지를 만들고 map · geo · pinvi · concierge · weather · transport를
  올려 **이미지 간 정합성을 맞추는 구조도 같다** — pinset 해시 계약·교차 저장소
  revision 핀·journal phase 게이트·receipt/permit 체인을 더 늘리지 않는다.
- 새 검사기·게이트·봉인을 붙이고 싶으면 먼저 멈추고, 그것 없이 성립하는 더 단순한
  설계를 먼저 본다. 추가한다면 무엇을 막는지와 **어떤 실패 모드를 새로 만드는지**를
  같이 적는다.
- 걷어내는 방향이 기본값이지만, 동작 중인 보장을 조용히 없애지는 않는다 — 무엇을
  잃는지 명시하고 사용자 판단을 받는다.

이 원칙이 나온 이유는 이론이 아니라 실측이다: 소비된 pinset 재회전 차단으로 인한
영구 고착, 봉인된 실패 메시지가 원인을 가린 진단 불가, `.env` 해시 게이트의 영구
고착, 4겹 중첩 해시의 연쇄 재봉인, role 하나 합치자 무효화된 상호 배타 게이트 35개+,
PinVi M05의 half-cutover 결함 다섯 — 전부 ADR-50에 근거와 함께 적혀 있다.
...
| DB 서비스 정보 | PostgreSQL / PostGIS 5개 — `kor-travel-geo-postgres`:**12500**(`kor_travel_geo`, `kor_travel_geo_dagster`, ADR-45 cutover 완료 전까지는 이 instance가 활성, 완료 후에는 롤백 보관용) · `kor-travel-concierge-postgres`:**12600**(`kor_travel_concierge` — ADR-44 cutover가 2026-09-19/20에 끝나 지금은 **롤백 보관용**이고 앱은 쓰지 않는다) · `kor-travel-map-postgres`:**12700**(`kor_travel_map`, `kor_travel_map_dagster`) · `pinvi-postgres`:**12800**(`pinvi`, `pinvi_dagster` — ADR-46 이후 **롤백 보관용**이고 앱은 쓰지 않는다) · `kor-travel-shared-postgres`:**11000**(`kor_travel_concierge` — concierge 전용 role, ADR-44 cutover 완료로 **현재 활성**; `kor_travel_geo`/`kor_travel_geo_dagster` — geo 전용 role `kor_travel_geo_app`, ADR-45 cutover 완료 후 활성(instance/role/database는 이미 존재, DSN 전환은 별도 단계); `pinvi`+`pinvi_dagster` — ADR-46, 데이터 보존 없이 fresh 구성으로 이전해 **현재 활성**. 새 프로젝트 합류 절차는 `docs/shared-postgres-onboarding.md`). map은 ADR-37 그대로 전용 instance를 유지하고, concierge(ADR-44)·geo(ADR-45)·PinVi(ADR-46)는 공용 instance(`:11000`)로 이전한다. 다섯 다 loopback 전용. 통합 instance(5432)는 2026-08-17 폐지(ADR-37) — **`5432`를 듣는 것은 없다**. |
| 파일 스토리지 정보 | RustFS (기본 host 포트: 12101 / 콘솔 host 포트: 12105) |
| 관측 서비스 정보 | Grafana `kor-travel-grafana`: 12104 / cAdvisor `kor-travel-cadvisor`: 12103 / Prometheus `kor-travel-prometheus`: 12102 (ADR-48로 `gra`/`cadv`/`prom` 자신의 100단위 대역이 아니라 `storage` 대역 안으로 재배치, `docs/ports.md` 참고) |
...
4. **포트 충돌 유발 금지**: PostgreSQL(`12500`/`12600`/`12700`/`12800` — geo/concierge/map/pinvi 전용 instance, ADR-37; `11000` — 공용 제어 평면 instance, ADR-44/ADR-45/ADR-46. concierge는 ADR-44로 `11000`쪽 role/database로 **이전을 마쳤고**(2026-09-19/20) `12600` instance는 롤백 보관용으로 남는다. geo는 ADR-45로 이전 준비(role/database 생성 코드)까지 끝났고 실제 data cutover는 별도 배포 단계다 — 그때까지 `12500` instance가 계속 활성이다. PinVi는 ADR-46으로 `11000`쪽으로 **이전을 마쳤고**(데이터 보존 없이 fresh 구성) `12800` instance는 롤백 보관용으로 남는다. map은 ADR-37 그대로 자기 instance를 쓴다), RustFS(`12101`, `12105`), Grafana(`12104`), cAdvisor(`12103`), Prometheus(`12102`)(이 셋은 2026-09-21 ADR-48로 `12205`/`12301`/`12401`에서 재배치됐다 — `docs/ports.md` 참고), `kor-travel-geo`(`12501`, `12505`), `kor-travel-concierge`(`12601`, `12602`, `12605`), `kor-travel-map`(`12701`, `12702`, `12705`), PinVi(`12801`, `12805`), Manager(`12901`, `12905`) 포트는 Kor Travel/PinVi 구성 프로그램이 공용으로 접근할 수 있어야 하므로 임의로 변경하지 않는다.
...
9. **포트 정책 우회 금지**: 새 로컬 서비스 포트는 `docs/ports.md`의 `12000 + dependency index * 100 + offset` 규칙을 따른다. **PostgreSQL은 프로젝트마다 전용 instance를 쓰고 포트는 각 대역의 `x00`이다(ADR-37)** — map은 지금도 이 규칙 그대로다. **concierge(ADR-44, 2026-09-19/20)와 PinVi(ADR-46, 데이터 보존 없이 fresh 구성)는 예외로 `11000` 공용 제어 평면 instance로 이전을 마쳤다. geo도 예외 대상으로 확정됐고(ADR-45) role/database 생성까지 끝났으나, 실제 data cutover는 아직이다** — `11000`은 target별 100단위 대역 밖의 별도 포트이며, 새 프로젝트가 이 instance에 합류해도 이 포트 자체는 바뀌지 않는다. 표준 `5432`를 쓰던 통합 instance는 2026-08-17에 폐지했다.
...
15. **한 사실을 두 곳에 독립 선언 금지**: 같은 사실(revision·digest·포트·역할 목록·workspace 목록·실행 비트 대상 등)을 두 곳 이상에 적어야 한다면 **유도 → 결박 → 탐지** 순으로 내려간다.
    1. **유도**: 사본을 지우고 정본에서 계산한다(glob·import·읽기). 가능하면 여기서 끝낸다.
    2. **결박**: 사본이 *필요한 이유가 있으면*(신뢰 경계 등) 남기되, 두 선언의 **집합 동등성**을 강제하는 테스트를 같은 PR에 넣고 그 테스트 이름을 사본 옆 주석에 적는다.
    3. **탐지**: 위 둘 다 못 하면 그 사실을 `docs/bindings.md`에 등록하고 왜 결박할 수 없는지 적는다.

    그리고 **결박은 그 사실이 깨질 수 있는 가장 이른 지점**에 둔다. 소비 시점(빌드·배포·e2e)이 아니라 선언 시점(회전·커밋·설정)이다. 2026-09-02에 회전 시점에 0초로 판정 가능한 모순이 71분짜리 rebuild를 태운 뒤에야 드러났다.

    도구 수준에서 **애매하면 실패하게** 하는 것이 결박 하나 추가보다 강하다: `npm install` 대신 `npm ci`, 태그 대신 digest 핀, lockfile 없으면 하드 실패.

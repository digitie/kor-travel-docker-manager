...
| DB 서비스 정보 | PostgreSQL / PostGIS 5개 — `kor-travel-geo-postgres`:**12500**(`kor_travel_geo`, `kor_travel_geo_dagster`) · `kor-travel-concierge-postgres`:**12600**(`kor_travel_concierge`, ADR-44 cutover 완료 전까지는 이 instance가 활성, 완료 후에는 롤백 보관용) · `kor-travel-map-postgres`:**12700**(`kor_travel_map`, `kor_travel_map_dagster`) · `pinvi-postgres`:**12800**(`pinvi`) · `kor-travel-shared-postgres`:**11000**(`kor_travel_concierge` — concierge 전용 role, ADR-44 cutover 완료 후 활성). geo/map/pinvi는 ADR-37 그대로 전용 instance를 유지하고, concierge만 ADR-44로 공용 instance(`:11000`)로 이전한다. 다섯 다 loopback 전용. 통합 instance(5432)는 2026-08-17 폐지(ADR-37) — **`5432`를 듣는 것은 없다**. |
...
9. **포트 정책 우회 금지**: 새 로컬 서비스 포트는 `docs/ports.md`의 `12000 + dependency index * 100 + offset` 규칙을 따른다. **PostgreSQL은 프로젝트마다 전용 instance를 쓰고 포트는 각 대역의 `x00`이다(ADR-37)** — geo/map/pinvi는 지금도 이 규칙 그대로다. **concierge만 예외로, `11000` 공용 제어 평면 instance로 이전했다(ADR-44)** — `11000`은 target별 100단위 대역 밖의 별도 포트이며, 새 프로젝트가 이 instance에 합류해도 이 포트 자체는 바뀌지 않는다. 표준 `5432`를 쓰던 통합 instance는 2026-08-17에 폐지했다.

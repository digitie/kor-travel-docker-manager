...
- [ ] GM-17 본작업 A — bind allowlist 외부화. `_CANDIDATE_ALLOWED_OPERATOR_BINDS`(c6c_deployment.py)를 targets 문서로 옮기고 코드에는 검증 규칙만 남긴다. **선행조건은 위에서 닫혔다.** 새 bind 하나에 backend 수정 + trusted release 재설치가 필요한 병목이 이것으로 풀린다. 보안 경계를 옮기는 작업이라 적대 리뷰 권장. **착수 전 오너와 범위를 재확인할 것.**

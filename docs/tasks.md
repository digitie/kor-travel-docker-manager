...
- [ ] T-VN-M05-ISOLATED-HARNESS — host-network canonical runtime을 변경하지 않고, exact Map/PinVi source·bridge network·fresh volume·loopback binding만 쓰는 root-only M04/M05 isolated harness를 추가한다. Docker mutation 전 `(harness, pinset, Manager revision)` ledger claim과 global lock을 고정하고, 증적은 production activation과 분리한다.

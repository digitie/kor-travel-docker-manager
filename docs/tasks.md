...
- [/] T-VN-M05-NEW-CANDIDATE — `6269138f…`는 n150에서 단 한 번 실행됐으나 durable journal/manifest 없이 pre-journal에서 끝났고 raw stderr는 열지 않는다. PinVi `55687a4f…`·Map `9c64e862…`의 `53d4639f…`만 다음 candidate로 고정한다. `run-pinned-rebuild-once`가 root-owned structured JSON result와 raw stderr 분리를 강제하므로 이 새 pinset에서만 다음 rebuild를 한 번 실행한다.

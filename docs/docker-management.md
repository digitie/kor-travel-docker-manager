...
candidate pair 전체 계약을 검증해 최초 v3를 만든다. provenance가 없는 v1/v2는
자동 전환하지 않고 거부한다.

```bash
ktdctl pinvi-pair capture --verified-compatible --build
# 기본 manifest: ~/.local/state/kor-travel-docker-manager/<COMPOSE_PROJECT_NAME>/compatible-pair-v3.json
```

capture는 host lock 안에서 base dependency → Map → signed smoke → Map UI/Dagster → PinVi →
canonical smoke → PinVi Web/Dagster → 전체 smoke 순서로 candidate를 검증한다. 실패하면 v3를 기록하지
않고 두 API를 중지하며 capture가 새로 만든 container만 제거한다. 기존 manifest는
덮어쓰지 않는다. manifest v3의 active/rollback은 각각 `map_image_id`,
`map_source_revision`, `pinvi_image_id`, `pinvi_source_revision`, `contract_generation`,
`recorded_at`의 exact 6개 필드를 기록한다. image ID는 `sha256:<64 hex>`, source revision은
...
1. deployment-wide host lock을 잡고 mode/token/base URL/generation, 단일 canonical base compose의
   ...
   mutation source로 허용하지 않는다. `--build`는 두 build context가 exact Git root이고
   worktree가 clean인지 검사한 뒤 각 lowercase 40자 `HEAD`를
   `KOR_TRAVEL_MAP_GIT_COMMIT`/`PINVI_SOURCE_REVISION`으로 파생하고 PinVi build mode를
   `production`으로 고정한다. 두 candidate image는 API `up`과 분리해 먼저 build하고 immutable
...
2. 현재 active pair와 공용 dependency·Map/PinVi UI·Dagster의 running/healthy를 확인한다. 현재 Map UI
   ...
   먼저 중지해 mixed pair 노출을 막은 뒤 `--no-deps`로 새 Map API image만 재생성한다.
   ...
   owned job에서 수행한다.
3. `--no-deps`로 새 PinVi API image만 재생성하고 image의
...
root와 `compatible-pair-v3.json`/`deployment.lock` 파일명을 고정하고 모든 path override를 거부해 같은

...
| DB 서비스 정보 | PostgreSQL / PostGIS 5개 — `kor-travel-geo-postgres`:**12500**(`kor_travel_geo`, `kor_travel_geo_dagster`) · `kor-travel-concierge-postgres`:**12600**(`kor_travel_concierge` — ADR-44 cutover가 2026-09-19/20에 끝나 지금은 **롤백 보관용**이고 앱은 쓰지 않는다) · `kor-travel-map-postgres`:**12700**(`kor_travel_map`, `kor_travel_map_dagster`) · `pinvi-postgres`:**12800**(`pinvi`) · `kor-travel-shared-postgres`:**11000**(`kor_travel_concierge` — concierge 전용 role, ADR-44 cutover 완료로 **현재 활성**. 새 프로젝트 합류 절차는 `docs/shared-postgres-onboarding.md`). geo/map/pinvi는 ADR-37 그대로 전용 instance를 유지하고, concierge만 ADR-44로 공용 instance(`:11000`)로 이전한다. 다섯 다 loopback 전용. 통합 instance(5432)는 2026-08-17 폐지(ADR-37) — **`5432`를 듣는 것은 없다**. |
...
4. **포트 충돌 유발 금지**: PostgreSQL(`12500`/`12600`/`12700`/`12800` — geo/concierge/map/pinvi 전용 instance, ADR-37; `11000` — 공용 제어 평면 instance, ADR-44. concierge는 ADR-44로 `11000`쪽 role/database로 **이전을 마쳤고**(2026-09-19/20) `12600` instance는 롤백 보관용으로 남는다. geo/map/pinvi는 ADR-37 그대로 자기 instance를 쓴다), RustFS(`12101`, `12105`), Grafana(`12205`), cAdvisor(`12301`), Prometheus(`12401`), `kor-travel-geo`(`12501`, `12505`), `kor-travel-concierge`(`12601`, `12602`, `12605`), `kor-travel-map`(`12701`, `12702`, `12705`), PinVi(`12801`, `12805`), Manager(`12901`, `12905`) 포트는 Kor Travel/PinVi 구성 프로그램이 공용으로 접근할 수 있어야 하므로 임의로 변경하지 않는다.
...
9. **포트 정책 우회 금지**: 새 로컬 서비스 포트는 `docs/ports.md`의 `12000 + dependency index * 100 + offset` 규칙을 따른다. **PostgreSQL은 프로젝트마다 전용 instance를 쓰고 포트는 각 대역의 `x00`이다(ADR-37)** — geo/map/pinvi는 지금도 이 규칙 그대로다. **concierge만 예외로, `11000` 공용 제어 평면 instance로 이전을 마쳤다(ADR-44, 2026-09-19/20)** — `11000`은 target별 100단위 대역 밖의 별도 포트이며, 새 프로젝트가 이 instance에 합류해도 이 포트 자체는 바뀌지 않는다. 표준 `5432`를 쓰던 통합 instance는 2026-08-17에 폐지했다.

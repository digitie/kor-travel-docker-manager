...
### 5-0. 실행할 수 없는 것은 판정해서 준다

`rebuild-pinned`처럼 **backend가 물리적으로 실행할 수 없는** 작업에는 버튼을 두지
않는다. 대신 화면이 "지금 눌러도 되는가"를 판정하고 실행할 명령을 준다
(`GET /api/v1/pinned-rebuild/preflight`, `SourceStatusPanel`의 "재구축 실행").

비전문 관리자에게 실제 장벽은 "SSH로 가라"가 아니라 **"가서 무엇을 쳐야 하고, 지금
쳐도 되는지"**다. 차단 사유를 사람 말로 보여 주고 정확한 명령을 복사하게 하면 그 장벽이
사라지면서도 파괴적 실행의 마찰은 그대로 남는다.

- **차단 상태에서도 명령을 숨기지 않는다.** 무엇을 실행하려던 것인지 보이지 않으면
  차단 사유와 연결 짓기 어렵다. 대신 "해소 전에는 실행하지 마세요"라고 쓴다.
- **차단이 아닌 것을 차단으로 만들지 않는다.** 옛 terminal 기록(`blocked_pinsets`)은
  ADR-51 뒤 재구축을 막지 않고 경고로만 남는다 — 화면도 "거부"로 쓰지 않는다(root 검증이
  필요하다는 "확인하지 못한 것"으로 보여 준다). journal 재개 경고(`attention` 상태)는
  재개가 없어지면서 ADR-51 B3에서 지웠다.
- **판정 근거를 하나라도 잃으면 초록불을 켜지 않는다.**
...
- **아무것도 바뀌지 않았다고 명시한다.** 버튼 문구는 "적용"이 아니라 "변경 요청 기록"이고,
  폼과 대기 카드 **양쪽에** 적용 명령을 함께 둔다(`RuntimePinPanel`의 `applyCommand`).
- **폼을 숨길 때는 이유를 함께 남긴다.** `status !== 'ok'`이면 폼 대신 "왜 지금은 받을 수
  없는지"와 해소 명령을 그 자리에 보여 준다. 아무 말 없이 사라지면 "왜 못 바꾸지"만 남는다.
- **화면에서 손댈 수 없는 상태에는 CLI 탈출구를 준다.** 요청 파일이 손상되면 id를 알 수
  없어 취소 버튼을 만들 수 없다 — 그때는 `pin clear-pending --force --confirm`을 준다.
...
전부 빨갛게 칠하면 진짜 차단 항목이 묻힌다. 대신 **`warn`이 있으면 요약 옆에 건수를
적는다** — 초록 요약 아래에 묻어 두면 스크롤하지 않는 사람은 영영 못 본다.

**캐시된 값은 캐시라고 말하고, 새로고침은 실제로 다시 관측해야 한다.** readiness는 서버에
30초 TTL 캐시가 있다. 화면이 그 사실을 숨기면, SSH에서 조치한 운영자가 새로고침을 몇 번
눌러도 같은 차단 문구를 보고 "조치가 실패했다"고 결론짓는다. `?refresh=true`로 TTL을
건너뛰고, 캐시된 값일 때는 관측 시각을 함께 쓴다.
...
- [ ] 서버가 거부할 입력은 편집 가능하게 두지 않았다 — 계약이 값을 고정한 환경변수는
      백엔드가 `config.locked_env`로 알려 주며, 편집기는 그것을 읽기 전용으로 만든다.
